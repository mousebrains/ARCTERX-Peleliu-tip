...
stimeJoint = datetime("2023-05-22 05:10:54"); % By inspection
etimeJoint = datetime("2023-05-23 06:34:00");
...
for index = 1:nDrifters
    tbl{index} = loadData(files.fn{index}, stimeJoint, etimeJoint);
    t0 = min(tbl{index}.time(1,1), t0);
    t1 = max(tbl{index}.time(end:end), t1);
end % for index
fprintf("Took %.2f seconds to load %d files\n", toc(tStart), nDrifters);

%% ------------------------------------------------------------------------
%  Apply lowpass filter to some of the variables to drop wave noise
%  ------------------------------------------------------------------------

tStart = tic();
for index = 1:nDrifters % lowpass doesn't like thread based pools
    a = tbl{index};
    for name = ["u", "v", "w", "lat", "lon", "sog", "cog", "height", "hmsl"];
        val = a.(name);
        a.(append("s_", name)) = lowpass(val', 1/60, 2, "ImpulseResponse","iir")';
    end % for name
    tbl{index} = a;
end % for index
fprintf("Took %.2f seconds to lowpass\n", toc(tStart));

%% ------------------------------------------------------------------------
%  flatten arrays
%  ------------------------------------------------------------------------

lat0 = cell(nDrifters,1);
lon0 = cell(nDrifters,1);
...
%% ------------------------------------------------------------------------
%  calculate distance from center lat/lon and the velocity direction
%  ------------------------------------------------------------------------

tStart = tic();
lat0 = mean(vertcat(lat0{:}), "omitmissing");
lon0 = mean(vertcat(lon0{:}), "omitmissing");
parfor index = 1:nDrifters
    a = tbl{index};
    [dist, az] = distance(lat0, lon0, a.s_lat, a.s_lon, wgs84Ellipsoid("meter"));
    a.s_x = dist .* cosd(90 - az);
    a.s_y = dist .* sind(90 - az);
    a.s_theta = atan2d(a.s_y, a.s_x);
    tbl{index} = a;
end % for index
fprintf("Took %.2f seconds to calculate x/y\n", toc(tStart));
...
for sindex = 1:nTimeWindows
    ...
    for index = 1:nDrifters
        a = tbl{index}(:,["time", "s_theta", "s_x", "s_y"]);
    ...
    pars0 = [mean(b.s_x), mean(b.s_y), repmat(radius0, 1, nDrifters)];
    error("GotMe");
...
plot(centroids.xCenter, centroids.yCenter, "o-");
hold off;
grid on;
...
%     x     array of the window's samples, with sx, sy (meters),
...
theta_perp = b.s_theta + 90; % Perpendicular to velocity direction
rx = radii(b.index) * cosd(theta_perp);
ry = radii(b.index) * sind(theta_perp);
dp = (xCenter - (b.s_x + rx)).^2 + (yCenter - (b.s_y + ry)).^2;
dm = (xCenter - (b.s_x - rx)).^2 + (yCenter - (b.s_y - ry)).^2;

dist = zeros(size(b,1),1);
for index = 1:numel(radii)
    q = b.index == index;
    dist(q) = dist(q) + min(dp(q), dm(q));
end % for index
dist = sum(dist);
end % myCentroid

function tbl = loadData(fn, stime, etime)
...
%   2048 samples per wave-processing period. 
%   There is a time gap between the blocks, 30-300 seconds.
%
%   For filtering, I'm not going to flatten, so we can do lowpass filter
%   for each block.
%
%   time is in milliseconds since 1970-01-01 UTC.
...
names = ["time", "u", "v", "w", "lat", "lon", "sog", "cog", "numsats", "pdop" ...
    "height", "hmsl", "h_acc", "v_acc", "s_acc", "head_acc", "t_acc", ...
    "flags", "flags2", "valid", "fix_type", "itow", "nano"];

fprintf("Reading %s\n", fn);

tbl = struct();
for name = names
    tbl.(name) = ncread(fn, name)';
end % for name

tbl = struct2table(tbl);

tbl.time = datetime(tbl.time/1000, ConvertFrom="posixtime");
tbl = tbl(tbl.time(:,1) >= stime & tbl.time(:,end) <= etime,:);
end % loadData
